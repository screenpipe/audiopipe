...
//! End-to-end test: load Qwen3-ASR from local dir and transcribe real audio.
...
fn main() {
    ...
    let model_dir = args.get(1).map(|s| s.as_str()).unwrap_or("qwen3-asr-0.6b-onnx-fp16");
    ...
    let mut model = stt::Model::from_dir(std::path::Path::new(model_dir), "qwen3-asr")
    ...
    let warmup_result = model.transcribe_with_sample_rate(&audio, sample_rate, stt::TranscribeOptions::default());
    ...
    let result = model.transcribe_with_sample_rate(&audio, sample_rate, stt::TranscribeOptions::default());
