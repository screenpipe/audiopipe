...
float *qwen_parse_wav_buffer(const uint8_t *data, size_t file_size, int *out_n_samples) {
    ...
    if (sample_rate != SAMPLE_RATE) {
        ...
        /* I0 (modified Bessel, first kind, order 0) — portable version */
        double inv_I0_beta = 1.0 / compat_bessel_i0(KAISER_BETA);
        ...
        for (int i = 0; i < new_n; i++) {
            ...
            for (int j = j_lo; j <= j_hi; j++) {
                ...
                if (npos <= -1.0 || npos >= 1.0) {
                    ...
                    w = compat_bessel_i0(KAISER_BETA * sqrt(1.0 - npos * npos)) * inv_I0_beta;
                }
